...
% Ce code Matlab implémente un modèle de contour actif
% D'application de la formulation de l'évolution de l'indice de
% régularisation à distance (DRLSE) dans le document de Li et al:
...
for k=1:iter
    ...
    Ny=phi_y./(s+smallNumber);%Ajoutez un petit nombre positif pour éviter la division par zéro
    curvature=div(Nx,Ny);
    ...
    else % Calculer le terme de régularisation de distance dans l'équation (13) avec le potentiel de double puits p2.
    ...
    areaTerm=diracPhi.*g; % balloon/pressure force % Ballon / force de pression
...
%Calculer le terme de régularisation de distance avec le potentiel double puits p2 dans l'équation (16)
[phi_x,phi_y]=gradient(phi);
s=sqrt(phi_x.^2 + phi_y.^2);
a=(s>=0) & (s<=1);
b=(s>1);
